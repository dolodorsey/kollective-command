  ...
  const { data: outbox = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("communications")
        .select("*").eq("channel", "email").eq("direction", "outbound")
      ...
      {composing && (
        <div className="rounded-lg border border-primary/20 bg-card p-4 space-y-2">
          <div className="grid grid-cols-2 gap-2">
            <input placeholder="To..." className="rounded-md border border-border/50 bg-input px-3 py-2 text-sm text-foreground outline-none placeholder:text-muted-foreground/40 focus:border-primary/40" />
            <input placeholder="Subject..." className="rounded-md border border-border/50 bg-input px-3 py-2 text-sm text-foreground outline-none placeholder:text-muted-foreground/40 focus:border-primary/40" />
          </div>
          <textarea rows={3} placeholder="Write your email..." className="w-full rounded-md border border-border/50 bg-input px-3 py-2 text-sm text-foreground outline-none placeholder:text-muted-foreground/40 focus:border-primary/40 resize-y" />
        ...
        <div className="flex w-[38%] flex-col border-r border-border/50">
          <div className="border-b border-border/50 px-4 py-2.5">
            <span className="text-[10px] font-semibold uppercase tracking-wider text-muted-foreground">
              {view === "inbox" ? `${threads.length} threads` : `${outbox.length} sent`}
            </span>
          </div>
            ...
            {view === "inbox" ? (
              threads.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-16">
                  ...
                  <p className="mt-3 text-xs text-muted-foreground/40">No email threads yet</p>
                </div>
              ) : threads.map((t: any) => (
              ...
              outbox.map((o: any, i: number) => (
                <div key={i} className="border-b border-border/20 px-4 py-3">
                  ...
                  <div className="text-[10px] text-muted-foreground/40 mt-0.5 truncate">{o.subject || o.message_text?.slice(0, 60)}</div>
          ...
          {selected ? (
            <>
              ...
              <div className="border-t border-border/50 p-3.5 flex gap-2">
                <input placeholder="Reply..." className="flex-1 rounded-md border border-border/50 bg-input px-3 py-2 text-sm text-foreground outline-none placeholder:text-muted-foreground/40 focus:border-primary/40" />
            ...
            <div className="flex flex-1 flex-col items-center justify-center gap-2">
              ...
              <p className="text-xs text-muted-foreground/30">Select a thread to view</p>
            </div>
          )}
        </div>
      </div>
